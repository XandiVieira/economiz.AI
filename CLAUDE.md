...
- **Test mail goes to `developer@economizaai.app` — never the main inbox.** The point is the RECIPIENT: emails generated by test activity (contact/beta submissions, admin alerts, verification codes) must land in the dedicated QA mailbox, not `contato@economizaai.app`. Two halves:
  ...
  - **Test account addresses**: register test users as `developer+<tag>@economizaai.app` (plus-alias for uniqueness, e.g. `developer+e2e-<timestamp>@…`) so their verification/notification mail also lands there. The Postman collection reads the box from the `testMailbox` collection variable.
