...
- **Test mail goes to `developer@economizaai.app` — never the main inbox.** The point is the RECIPIENT: emails generated by test activity (contact/beta submissions, admin alerts, verification codes) must land in the dedicated QA mailbox, not `contato@economizaai.app`. Two halves:
  ...
  - **Test account addresses**: register throwaway test users as `developer+<tag>@economizaai.app` (unique per run; the Postman collection reads the box from the `testMailbox` collection variable). CAVEAT: the mailbox provider (Titan/GoDaddy) does NOT deliver plus-aliases — mail to them bounces silently. When a test must actually RECEIVE the e-mail (report delivery, verification codes), use the shared QA account registered with the exact address `developer@economizaai.app` instead of registering a new one.
